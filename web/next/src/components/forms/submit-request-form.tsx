            ...
            <div className="grid gap-4">
              ...
              {selectedService ? (
                <div className="rounded-lg border border-[var(--line)] bg-slate-50 p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{selectedService.description}</p>
                  <p className="mt-3 text-xs font-semibold text-[var(--muted)]">Required: {selectedService.requiredDocuments.join(", ")}</p>
                </div>
              ) : null}
              <div className="rounded-md border border-amber-100 bg-amber-50 p-4 text-sm leading-6 text-amber-950">
                Final price is confirmed after checking documents, portal steps, and deadline.
              </div>
            </div>
          ) : null}
