    ...
    <section className="color-strip rounded-lg border border-[var(--line)] bg-white p-4 pt-6 shadow-sm">
      <div className="grid gap-3 lg:grid-cols-[1fr_240px]">
        <label className="relative">
          ...
          <input
            className="focus-ring min-h-11 w-full rounded-md border border-[var(--line)] bg-white px-3 pl-10 text-sm"
        ...
        <label>
          ...
          <select
            className="focus-ring min-h-11 w-full rounded-md border border-[var(--line)] bg-white px-3 text-sm font-semibold"
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[1080px] border-collapse text-left text-sm">
          <thead>
            <tr className="border-y border-[var(--line)] bg-slate-50 text-xs uppercase text-[var(--muted)]">
          ...
          <tbody className="divide-y divide-[var(--line)]">
            {filteredRequests.map((request) => (
              <tr className="transition hover:bg-blue-50/50" key={request.id}>
                ...
                <td className="px-3 py-4">
                  <span className={`rounded-full px-2.5 py-1 text-xs font-bold ring-1 ${request.urgency === "urgent" ? "bg-amber-50 text-amber-900 ring-amber-200" : "bg-slate-100 text-slate-700 ring-slate-200"}`}>
                    {request.urgency === "urgent" ? "High" : "Normal"}
                  </span>
                </td>
                <td className="px-3 py-4">
                  <p className="text-xs font-semibold text-[var(--muted)]">
                    {request.documents.filter((document) => document.status === "received").length}/{request.documents.length} received
                  </p>
                  <p className="mt-1 text-xs text-[var(--muted)]">
                    {request.documents.some((document) => document.status !== "received") ? "Needs check" : "Ready"}
                  </p>
                </td>
                ...
                <td className="px-3 py-4">
                  <Link className="focus-ring rounded-md bg-blue-50 px-3 py-2 text-sm font-bold text-[var(--trust-dark)] hover:bg-blue-100" href={`/admin/requests/${request.requestId}`}>
                    Open
                  </Link>
                </td>
              </tr>
